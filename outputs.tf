...
output "f5xc_lb_name" {
  description = "F5XC HTTP LB name"
  value       = volterra_http_loadbalancer.app_lb.name
}

output "f5xc_origin_pool_name" {
    ...
    Client (${var.client_subnet_cidr})
      → VPC Peer
        → NLB VIP: ${google_compute_forwarding_rule.ce_sli_nlb.ip_address} (all ports)
          → CE Site1 or Site2 SLI (eth1)
            → F5XC HTTP LB (VIP: ${var.vip_address}, domain: ${var.app_domain})
              → Origin Pool → SNAT to ${var.snat_pool_prefix}
                → App Server: ${var.app_server_ip}:${var.app_server_port} (${var.app_subnet_cidr})
  EOT
}

# ---- Console URLs ----
output "f5xc_console_sites_url" {
  description = "F5XC console URL to check site registration status"
  value       = "https://your-tenant.console.ves.volterra.io/web/workspaces/multi-cloud-network-connect/overview/sites"
}
